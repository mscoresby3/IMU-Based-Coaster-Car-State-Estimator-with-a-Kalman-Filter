...
def main(step, comp_ratio):
    ...
    der_angle_points_noise = noise.bias_noise(der_angle_points, step / 2)
    ...
    row2_col2_fig.plot(time_points, comp_filter_points, label='Filtered data')
    row2_col2_fig.set_title('Complementary filter')
    row2_col2_fig.legend()

    row2_col3_fig.plot(time_points, [comp_filter_points[i] - angle_points[i] for i in range(len(angle_points))], label='Angle Diff')
    row2_col3_fig.set_ylim(-1, 1)
    row2_col3_fig.set_title('Comp filter Diff')
